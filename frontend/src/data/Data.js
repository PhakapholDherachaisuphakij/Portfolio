...
export const portfoliodata = [
  ...
  },
  {
    ...
    description: "A personal knowledge management workspace synchronized with my local Obsidian Markdown Vault, integrated with Typhoon 30B AI Copilot (Hermes), under a zero-trust serverless architecture and central approval flow via PK Brain.",
    ...
    experience: "Engineering Case Study & Insights:\n\n1. Challenges Encountered:\n- I maintain my academic notes for KMUTT and SCB QA documentation in Obsidian on my local machine. When opening this workspace for public access or remote usage, I faced several security and infrastructure risks:\n  - Unauthorized usage of my Typhoon 30B AI API quota from public visitors or malicious bots.\n  - Vulnerability to spam account registration and rate-limit depletion.\n  - Availability risks associated with home server outages or local network interruptions, which would cause complete website downtime if directly proxied.\n\n2. Architecture & Engineering Solutions:\n- Zero-Trust Serverless Auth & Centralized PK Brain Approval: I completely removed default admin PIN backdoors. Access requests (Name, Email, Password, Reason) are submitted via Vercel serverless functions directly to Supabase Cloud. Approval is strictly granted through my private PK Brain Hub running on Tailscale. Unauthenticated users cannot trigger AI endpoints or edit content.\n- Zero-Downtime Static Snapshot Fallback: I developed an automated build script (generate_static_vault.js) that compiles my local Obsidian Markdown vault into an embedded static snapshot on Vercel. If the home server connection fails or exceeds 2.5 seconds, the application instantly falls back to the static snapshot in 0ms, guaranteeing 24/7 availability for public readers.\n- Notion-Style Block Editor & Mobile UX: Built a block editor supporting slash commands (/h1, /code, /table, /todo), a private note lock toggle for sensitive documents, and a responsive sliding drawer navigation optimized for mobile devices.",
